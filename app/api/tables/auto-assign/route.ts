...
export async function POST(request: NextRequest) {
  try {
    ...
    // Solo ospiti a cerimonia completa e confermati partecipano alla disposizione
    const eligible = await db
      .prepare(
        `SELECT id, family_id, table_id FROM guests
         WHERE invitation_type = 'full' AND response_status = 'confirmed'`
      )
      .all();
